...
use ice_constants, only : gravit, vonkar
...
if(rich_crit.le.0.25) call abort_ice ('ice: rich_crit in monin_obukhov_mod must be > 0.25')

!!if(drag_min.le.0.0)  call error_mesg( &
!!        'MONIN_OBUKHOV_INIT in MONIN_OBUKHOV_MOD', &
!!        'drag_min in monin_obukhov_mod must be >= 0.0', FATAL)
if(drag_min.le.0.0) call abort_ice ('ice: drag_min in monin_obukhov_mod must be >= 0.0')

!!if(stable_option < 1 .or. stable_option > 2) call error_mesg( &
!!        'MONIN_OBUKHOV_INIT in MONIN_OBUKHOV_MOD', &
!!        'the only allowable values of stable_option are 1 and 2', FATAL)
if(stable_option < 1 .or. stable_option > 2) &
          call abort_ice ('ice: the only allowable values of stable_option are 1 and 2')

!!if(stable_option == 2 .and. zeta_trans < 0) call error_mesg( &
!!        'MONIN_OBUKHOV_INIT in MONIN_OBUKHOV_MOD', &
!!        'zeta_trans must be positive', FATAL)
if(stable_option == 2 .and. zeta_trans < 0) &
          call abort_ice ('ice: zeta_trans must be positive')
...
!!   call monin_obukhov_drag_1d(grav, vonkarm,               &
   call monin_obukhov_drag_1d(gravit, vonkar,               &
...
!!   call monin_obukhov_profile_1d(vonkarm, &
   call monin_obukhov_profile_1d(vonkar, &
...
call abort_ice('ice: solve_zeta in monin_obukhov_mod -- surface drag iteration did not converge')
