...
use ice_constants, only : cp_air, Lvap, stefan_boltzmann, gravit, vonkar
use ice_constants, only : rdgas, rvgas   !newly added
...
real, parameter :: hlars  = Lvap/rvgas
!!real, parameter :: gcp    = grav/cp_air
real, parameter :: gcp    = gravit/cp_air
...
!bi003---
!if  (.not.ncar_ocean_flux .and. .not.ncar_ocean_flux_orig) then
!  !  monin-obukhov similarity theory 
!  call mo_drag (thv_atm, thv_surf, z_atm,                  &
!       rough_mom, rough_heat, rough_moist, w_atm,          &
!       cd_m, cd_t, cd_q, u_star, b_star, avail             )
...
!bi003: suspicious--rough_scale can NOT be "dimensionless" as claimed above!
!       to make sense of the equation regarding "scaling", i think this line shoule be like

     cd_m = cd_m*(log(z_atm/rough_mom+1)/log(z_atm/(rough_scale*rough_mom)+1))**2
...
!!     drdt_surf = 4*stefan*t_surf**3               ! d(upward longwave)/d(surface temperature)
     flux_r    =   stefan_boltzmann*t_surf**4       ! (W/m**2)stefan_boltzmann
     drdt_surf = 4*stefan_boltzmann*t_surf**3       ! d(upward longwave)/d(surface temperature)
...
!  write(41,'(10e12.4)') flux_t
!  write(42,'(10e12.4)') flux_q
!  write(43,'(10e12.4)') flux_r
!  write(44,'(10e12.4)') flux_u
!  write(45,'(10e12.4)') flux_v
...
!  open(unit=109,file="input_ice_gfdl.nml",form="formatted",status="old")
!  read (109, surface_flux_nml)
!  close(unit=109)
...
!!                bstar(i) = grav*(tstar/tv+qstar/(q(i)+1/0.608));
                bstar(i) = gravit*(tstar/tv+qstar/(q(i)+1/0.608));
!!                zeta     = vonkarm*bstar(i)*z(i)/(ustar(i)*ustar(i)); ! L-Y eqn. 8a
                zeta     = vonkar*bstar(i)*z(i)/(ustar(i)*ustar(i)); ! L-Y eqn. 8a
...
!!                u10 = u/(1+cd_n10_rt*(log(z(i)/10)-psi_m)/vonkarm);       ! L-Y eqn. 9
                u10 = u/(1+cd_n10_rt*(log(z(i)/10)-psi_m)/vonkar);       ! L-Y eqn. 9
